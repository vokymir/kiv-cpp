...
int main(int argc, char **argv) {
  ...
  if (!std::filesystem::exists(InFileName)) {
    ...
    std::cout << "Celkovy cas na analyzu "
              << (a->Get_File_Size() / (1024 * 1024))
              << " MiB dat a vygenerovani obrazku " << ImgWidth << "x"
              << ImgHeight << ": " << dur << "ms" << std::endl;
