...
public:
  ...
  char Read() {
    ...
    std::unique_lock<std::mutex> lk(mMutex);
    mCanRead.wait(lk, [this] { return mWrite_Cursor != mRead_Cursor; });
...
void Producent_Thread(StupidPipe &r, char c, size_t delay) {
  // TODO: synchronizujte vypisy tak, aby se v konzoli neprekryvaly a
  // neprokladaly, tedy aby se nezamichaly do sebe
